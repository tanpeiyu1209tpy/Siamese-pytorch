...
if __name__ == "__main__":
    ...
    #   请修改为你实际清洗后的数据根目录！
    #----------------------------------------------------#
    dataset_path    = "/kaggle/working/siamese_data_cleaned"
    #----------------------------------------------------#
    #   输入图像的大小，默认为105,105,3
    #----------------------------------------------------#
    input_shape     = [224, 224]
    
    # train_own_data 参数已废弃，删除
    ...
    #   同时修改下方的 冻结阶段 或者 解冻阶段 的参数，来保证模型epoch的连续性。
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = "/kaggle/input/vggpretrainpth/vgg16-397923af.pth"
    ...
    #----------------------------------------------------#
    #   修改点：直接实例化自定义数据集
    #   不再使用 load_dataset 和 train_ratio
    #----------------------------------------------------#
    # 注意：这里为了简化，训练集和验证集使用了同一个数据目录。
    # 在正式训练中，建议你划分出一个独立的验证集目录传入 val_dataset
    train_dataset   = SiameseDataset(input_shape, dataset_path, random_flag=True, autoaugment_flag=True)
    val_dataset     = SiameseDataset(input_shape, dataset_path, random_flag=False, autoaugment_flag=False)
    
    num_train   = len(train_dataset)
    num_val     = len(val_dataset)

    if local_rank == 0:
        # 移除了 show_config 中不再适用的参数
        show_config(
            model_path = model_path, input_shape = input_shape, \
            Init_Epoch = Init_Epoch, Epoch = Epoch, batch_size = batch_size, \
            Init_lr = Init_lr, Min_lr = Min_lr, optimizer_type = optimizer_type, momentum = momentum, lr_decay_type = lr_decay_type, \
